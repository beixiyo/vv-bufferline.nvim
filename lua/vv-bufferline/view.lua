...
local function update_hover()
  ...
  cancel_hover_clear()
  local changed = State.set_hovered(assert(win), assert(buf))
...
function M.refresh()
  ...
  if is_tabline_target() then
    ...
    for _, w in ipairs(vim.api.nvim_tabpage_list_wins(0)) do
      WinbarHost.restore(w)
      WinbarHost.clear_orphan(w)
    end
...
function M.interaction_win()
  if not is_tabline_target() then
    ...
    return Window.should_show(cur) and cur or nil
  end

  local cur = vim.api.nvim_get_current_win()
  if Window.should_show(cur) then
    last_editor_win = cur
    return cur
  end

  if last_editor_win and vim.api.nvim_win_is_valid(last_editor_win) and Window.should_show(last_editor_win) then
